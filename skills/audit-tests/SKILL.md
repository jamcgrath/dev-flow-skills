...
1. **Gather.** Acceptance criteria (`.dev-flow/<task>/TICKET_CONTEXT.md` if it exists, else the
   ...
   (`.dev-flow/<task>/PLAN_BRIEF.md`, or `BUG_CONTEXT.md` on the bug path — it carries the same
   section, since a bug run writes no `PLAN_BRIEF.md`) — you need it to know **how each runner reports
   an assertion failure vs an error**, which the whole audit turns on. If neither file is there or
   neither names the runner's failure format, **don't guess it** — say so and grade only what you can
